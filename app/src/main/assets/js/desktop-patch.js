...
body.glass-mode.glass-optimized .btn,body.glass-mode.glass-optimized .diff-btn {
  ...
  // ═══════════════════════════════════════════════════════════════
  // MutationObserver для lazy-load click-фиксов (patch mcRenderStickerPanel перенесён в блок 10)
  (function(){
    var _obs8 = new MutationObserver(_fixEmojiClicks);
    function _hookPanel8(){ var p=document.getElementById('mc-sticker-panel'); if(p) _obs8.observe(p,{childList:true,subtree:true}); }
    if (document.readyState!=='loading') _hookPanel8();
    else document.addEventListener('DOMContentLoaded', _hookPanel8);
  })();
  ...
  // ЕДИНСТВЕННЫЙ патч — блок 8 больше НЕ патчит эту функцию.
  _waitFor(function(){ return typeof window.mcRenderStickerPanel === 'function'; }, function(){
    ...
    window.mcRenderStickerPanel = function(){
      ...
      } else if ((window._stickerPanelTab || 'emoji') === 'emoji') {
        _oRender.apply(this, arguments);
      }
      setTimeout(_fixEmojiClicks, 80);
      setTimeout(_fixEmojiClicks, 300);
    };
    _log('mcRenderStickerPanel patched (единственный)');
  });

  // Патч mcToggleStickerPanel: fixed-панель вместо flex-потомка
  _waitFor(function(){ return typeof window.mcToggleStickerPanel === 'function'; }, function(){
    ...
    window.mcToggleStickerPanel = function(){
      ...
      if (window._mcStickerPanelOpen) {
        _spShow();
      } else {
        _spHide();
      }
    };
    _log('mcToggleStickerPanel patched (fixed position)');
  });

  // ── Показываем панель position:fixed поверх чата (решает overflow:hidden родителя) ──
  function _spShow(){
    var panel = document.getElementById('mc-sticker-panel');
    if (!panel) return;
    var bar   = document.getElementById('mc-input-bar');
    var offset = 0;
    if (bar) {
      var bRect = bar.getBoundingClientRect();
      offset = window.innerHeight - bRect.top;
    }
    // Применяем fixed-стиль
    var css = 'position:fixed;left:0;right:0;bottom:' + offset + 'px;z-index:450;'
      ...
      + 'will-change:transform;box-shadow:0 -4px 24px rgba(0,0,0,.35)';
    panel.style.cssText = css;

    if (!document.getElementById('_sp-tabbar')) {
      _spBuildLayout(panel);
    }
    // Slide-up анимация в следующем кадре
    requestAnimationFrame(function(){
      requestAnimationFrame(function(){
  ...
  function _spHide(){
    ...
    setTimeout(function(){
      if (panel) { panel.style.display = 'none'; panel.style.transform = ''; }
    }, 270);
  }

  function _spBuildLayout(panel){
